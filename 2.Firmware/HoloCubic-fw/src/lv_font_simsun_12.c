/**
 * @file lv_font_simsun_12.c
 * @brief LVGL宋体12像素字体文件
 * 
 * 功能概述：
 * 本文件包含了12像素大小的宋体字体数据，为HoloCubic提供中文
 * 显示支持。字体数据经过优化，包含常用的ASCII字符和中文汉字，
 * 确保在240x240像素的小屏幕上具有良好的可读性。
 * 
 * 字体特性：
 * 1. 字体大小：12像素高度，适合小屏幕显示
 * 2. 字体类型：宋体（SimSun），经典的中文字体
 * 3. 颜色深度：4位每像素（16级灰度），平衡显示效果和内存占用
 * 4. 字符集：包含ASCII字符和常用中文汉字
 * 
 * 技术规格：
 * - 像素大小：12px高度
 * - 位深度：4 BPP（Bits Per Pixel）
 * - 编码格式：Unicode支持
 * - 存储格式：压缩位图数据
 * - 内存优化：使用LVGL字体格式减少RAM占用
 * 
 * 字符支持：
 * - 基本ASCII字符（33-126）
 * - 数字和标点符号
